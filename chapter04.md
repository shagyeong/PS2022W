...
### 4.3.5 함수형 그래프
#### 함수형 그래프
함수형 그래프: 모든 노드의 출력 차수가 1인 그래프  
#### 희소배열 점화식
```
spar[k][j]=spar[spar[k][j-1]][j-1];
```
#### 희소배열 입력 예제
<img src="./static/PS435-sparsetable.png">

|N/L|$2^0$|$2^1$|$2^2$|$\cdots$|
|---|---|---|---|---|
|1|2|3|5|
|2|3|4|2|
|3|4|5|3|
...
|$\vdots$|
#### 희소배열
$5=(2^0\times1)+(2^1\times0)+(2^2\times1)$  
$f^5(1)\Rightarrow(\mathrm{spar}[1][0]=2)\Rightarrow(\mathrm{spar}[2][2]=2)$  
```
$ ./test
6
2 3 4 5 2 5
...
void  clean(void);
void sparse(void);
int   query(int u,int e);

int main(void){
    int j;
    int u; int e;
    scanf("%d",&n); init();
    for(j=1;j<=n;j++){scanf("%d",&spar[j][0]);}
    sparse();
    scanf("%d %d",&u,&e); printf("%d",query(u,e));
    clean();
}

void   init(void){
    int j; int k=0;
    spar=(int**)calloc((n+1),  sizeof(int*));
    _spar=(int*)calloc((n+1)*L,sizeof(int));
    for(j=0;j<=n;j++){spar[j]=_spar+k; k+=L;}
}
void  clean(void){
    free(_spar);
    free(spar);
...
int   query(int u,int e){
    int j;
    for(j=0;j<L;j++){if(((e>>j)&1)==1){u=spar[u][j];}}
    return u;
}
```
#### RMQ 입력 예제
<img src="./static/PS435-RMQ.png">

$\min(\{A_i|2\leq i\leq 5\})=\min(\mathrm{spar}[2][2],\mathrm{spar}[5-2^2+1][2])$  
길이가 $2^2$인, 원본 배열에서 오버랩되는 구간: $\{20,70,10,80\},\{70,10,70,60\}$  
#### RMQ
```
$ ./test
8 1
40 20 70 10 80 60 50 30
2 5
10
```
```C
...
int main(void){
    ...
    int l; int r;
    scanf("%d %d",&n,&q); init();
    for(j=1;j<=n;j++){scanf("%d",&spar[j][0]);} sparse();
    for(j=1;j<=n;j++){for(k=0;k<L;k++){printf("%d\t",spar[j][k]);}printf("\n");}
...
void sparse(void){
    ...
    for(k=1;k<=n;k++){
        ...
        if(spar[k][j-1]<spar[k+(1<<(j-1))][j-1]){spar[k][j]=spar[k][j-1];}
        else                                    {spar[k][j]=spar[k+(1<<(j-1))][j-1];}
    }}
}
int   query(int l,int r){
    int e=0; // 구간 길이보다 짧거나 같은 2의 거듭제곱
    while((1<<(e+1))<=(r-l+1)){e++;}
    if(spar[l][e]<spar[r-(1<<e)+1][e]){return spar[l         ][e];}
    else                              {return spar[r-(1<<e)+1][e];}
}
void   init(void){
    int j; int k=0;
    spar=(int**)malloc(sizeof(int*)*(n+1));
    _spar=(int*)malloc(sizeof(int) *(n+1)*L);
    for(j=0;j<=n;j++){spar[j]=_spar+k; k+=L;}
}
void  clean(void){
    free(_spar);
